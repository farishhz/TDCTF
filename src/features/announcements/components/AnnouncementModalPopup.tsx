...
export default function AnnouncementModalPopup({
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-[999] flex items-center justify-center p-3.5 sm:p-6 overflow-y-auto">
        ...
        <motion.div
          ...
          className="relative z-10 w-full max-w-xl overflow-hidden rounded-2xl sm:rounded-3xl border border-white/15 bg-[#0b101b]/98 text-gray-100 shadow-[0_25px_70px_rgba(0,0,0,0.85)] backdrop-blur-2xl"
        >
          {/* Close Button */}
          {announcement.is_dismissible && (
            <button
              ...
              className="absolute top-3.5 right-3.5 z-30 flex h-8 w-8 items-center justify-center rounded-full bg-black/60 backdrop-blur-md border border-white/15 text-gray-300 hover:text-white hover:bg-white/20 transition-all active:scale-95"
              aria-label="Tutup"
            >
              <X className="h-4 w-4" />
            </button>
          ...
          <div className="p-5 sm:p-7 space-y-4">
            ...
            <div className="flex items-center gap-2">
              <span
                className={`select-none text-[11px] font-bold uppercase tracking-wider px-2 py-0.5 rounded border ${typeStyle.className}`}
              >
                {typeStyle.label}
              </span>
            ...
            <div className="max-h-[50vh] overflow-y-auto pr-1 text-xs sm:text-sm text-gray-300 leading-relaxed space-y-3 prose prose-invert prose-p:my-1.5 prose-headings:text-white prose-headings:font-bold prose-code:bg-black/50 prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded prose-code:border prose-code:border-white/10 prose-code:text-blue-300 prose-ul:my-1.5 prose-li:my-0.5 max-w-none">
            ...
            <div className="flex items-center justify-end gap-2.5 pt-4 border-t border-white/10">
              <Button
                ...
                className="h-9 px-4 rounded-xl border border-white/15 bg-white/5 hover:bg-white/10 text-xs font-semibold text-gray-200 transition-all active:scale-[0.98]"
              >
              ...
              {announcement.cta_text && announcement.cta_link && (
                <Button
                  ...
                  className="h-9 px-4 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-xs font-semibold text-white shadow-lg shadow-blue-500/25 flex items-center gap-1.5 transition-all active:scale-[0.98]"
                >
